...
namespace cytnx_core {
  ...
  template <typename DType>
  std::shared_ptr<Storage_base> StorageImplementation<DType>::_create_new_sametype() {
    std::shared_ptr<Storage_base> out(new StorageImplementation());
    return out;
  }

  template <typename DType>
  std::shared_ptr<Storage_base> StorageImplementation<DType>::clone() {
    std::shared_ptr<Storage_base> out(new StorageImplementation());
  ...
  void StorageImplementation<DType>::Move_memory_(const std::vector<cytnx_uint64> &old_shape,
    ...
    std::shared_ptr<Storage_base> tmp(this);
    if (this->device == Device.cpu) {
  ...
  template <typename DType>
  std::shared_ptr<Storage_base> StorageImplementation<DType>::Move_memory(
...
// here, cpu->gpu with gid=device
  ...
  template <typename DType>
  std::shared_ptr<Storage_base> StorageImplementation<DType>::to(const int &device) {
...
// here, cpu->gpu with gid=device
  ...
  template <typename DType>
  std::shared_ptr<Storage_base> StorageImplementation<DType>::real() {
    if constexpr (!is_complex_v<DType>) {
      ...
      if (this->device == Device.cpu) {
        std::shared_ptr<Storage_base> out(new StorageImplementation<ValueType>());
  ...
  template <typename DType>
  std::shared_ptr<Storage_base> StorageImplementation<DType>::imag() {
    if constexpr (!is_complex_v<DType>) {
      cytnx_error_msg(true, "[ERROR] Storage.real() can only be called from complex type.%s", "\n");
      return nullptr;
    } else {
