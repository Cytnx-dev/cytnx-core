...
namespace cytnx_core {
  ...
  std::shared_ptr<Storage_base> Storage_base::astype(const unsigned int &dtype) {
    std::shared_ptr<Storage_base> out(new Storage_base());
    if (dtype == this->dtype) return std::shared_ptr<Storage_base>(this);
  ...
  std::shared_ptr<Storage_base> Storage_base::_create_new_sametype() {
    cytnx_error_msg(1, "%s", "[ERROR] call _create_new_sametype in base");
    return nullptr;
  }

  std::shared_ptr<Storage_base> Storage_base::clone() {
    std::shared_ptr<Storage_base> out(new Storage_base());
    return out;
  }
  ...
  std::shared_ptr<Storage_base> Storage_base::Move_memory(
  ...
  std::shared_ptr<Storage_base> Storage_base::to(const int &device) {
  ...
  void Storage_base::GetElem_byShape_v2(std::shared_ptr<Storage_base> &out,
  ...
  void Storage_base::GetElem_byShape(std::shared_ptr<Storage_base> &out,
                                     const std::vector<cytnx_uint64> &shape,
                                     const std::vector<cytnx_uint64> &mapper,
  ...
  // This is deprecated !!
  void Storage_base::SetElem_byShape(std::shared_ptr<Storage_base> &in,
                                     const std::vector<cytnx_uint64> &shape,
                                     const std::vector<cytnx_uint64> &mapper,
                                     const std::vector<cytnx_uint64> &len,
                                     const std::vector<std::vector<cytnx_uint64>> &locators,
  ...
  void Storage_base::SetElem_byShape_v2(std::shared_ptr<Storage_base> &in,
                                        const std::vector<cytnx_uint64> &shape,
                                        const std::vector<std::vector<cytnx_uint64>> &locators,
...
#pragma omp parallel for schedule(dynamic)
  ...
  std::shared_ptr<Storage_base> Storage_base::real() {
    cytnx_error_msg(true, "[ERROR] trying to call Storage.real() from void Storage%s", "\n");
  }
  std::shared_ptr<Storage_base> Storage_base::imag() {
  ...
  // bool Storage_base::approx_eq(const std::shared_ptr<Storage_base> &rhs,
