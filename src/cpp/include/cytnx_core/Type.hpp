...
#include <cytnx_core/errors/cytnx_error.hpp> // also brings in cuComplex.h
...
template <class T> using vec3d = std::vector<std::vector<std::vector<T>>>;
...
namespace internal {
template <class> struct is_complex_impl : std::false_type {};

template <class T> struct is_complex_impl<std::complex<T>> : std::true_type {};

template <typename> struct is_complex_floating_point_impl : std::false_type {};

template <typename T>
struct is_complex_floating_point_impl<std::complex<T>>
...
template <typename V, template <typename> class Transform>
using make_variant_from_transform_t =
...
template <typename T>
using is_complex_floating_point =
    internal::is_complex_floating_point_impl<std::remove_cv_t<T>>;
...
// is_complex_floating_point_v<T> is a template constant that is true if T is of
// type complex<U> where U is a floating point type, and false otherwise.
template <typename T>
constexpr bool is_complex_floating_point_v =
    is_complex_floating_point<T>::value;

// variant_index<T, Variant> returns the index of type T in the Variant, or
...
struct variant_index<T, std::variant<U, Types...>> {
  static constexpr size_t value =
      1 + variant_index<T, std::variant<Types...>>::value;
};
...
template <typename T> constexpr int type_size = sizeof(T);
template <> constexpr int type_size<void> = 0;
} // namespace internal

// the list of supported types. The dtype() of an object is an index into this
// list. std::variant works better than std::tuple here since a variant is
// constrained to only hold each type once, and we have
// std::variant_alternative_t<n> to get the n'th type, as well as the
// variant_index_v helper to get the index of a given type
using Type_list =
    std::variant<void, cytnx_complex128, cytnx_complex64, cytnx_double,
                 cytnx_float, cytnx_int64, cytnx_uint64, cytnx_int32,
                 cytnx_uint32, cytnx_int16, cytnx_uint16, cytnx_bool>;

// For GPU storage, the types are slightly different because CUDA uses their own
// complex type
#ifdef UNI_GPU
using Type_list_gpu =
    std::variant<void, cuDoubleComplex, cuComplex, cytnx_double, cytnx_float,
                 cytnx_int64, cytnx_uint64, cytnx_int32, cytnx_uint32,
                 cytnx_int16, cytnx_uint16, cytnx_bool>;
#endif
...
template <typename T> constexpr char *Type_names = nullptr;
template <> constexpr const char *Type_names<void> = "Void";
template <>
constexpr const char *Type_names<cytnx_complex128> =
    "Complex Double (Complex Float64)";
template <>
constexpr const char *Type_names<cytnx_complex64> =
...
template <typename T> constexpr char *Type_enum_name = nullptr;
...
struct Type_struct {
  const char *name; // char* is OK here, it is only ever initialized from a
                    // string literal
...
template <typename T> struct Type_struct_t {
  ...
  static constexpr Type_struct construct() {
    return {name,     enum_name, is_unsigned, is_complex,
            is_float, is_int,    typeSize};
  }
};
...
constexpr auto make_type_array_helper(std::index_sequence<Indices...>) {
  return std::array<Type_struct, sizeof...(Indices)>{Type_struct_t<
      std::variant_alternative_t<Indices, Variant>>::construct()...};
}
template <typename Variant> constexpr auto make_type_array() {
...
public:
  ...
  // This could be constexpr returning constexpr char*, but there is lots of
  // code that assumes that it returns a std::string and calls
  // getname(n).c_str()
  ...
  template <class T> static constexpr unsigned int cy_typeid(const T &rc) {
    return cy_typeid_v<T>;
  }

  // Find a common type for typeL and typeR
  static constexpr unsigned int type_promote(unsigned int typeL,
                                             unsigned int typeR) {
    if (typeL < typeR) {
      if (typeL == 0)
        return 0;
  ...
  using type_promote_t =
      std::variant_alternative_t<Type_class::type_promote(
                                     variant_index_v<TL, Type_list>,
                                     variant_index_v<TR, Type_list>),
                                 Type_list>;

  // Helper to promote two pointer types (note does _not_ return another pointer
  // type)
  template <typename TL, typename TR> struct type_promote_from_pointer {
    using type = void;
  };
  ...
  // helper typedef
  template <typename TL, typename TR>
  using type_promote_from_pointer_t =
  ...
  using type_promote_gpu_t =
      std::variant_alternative_t<Type_class::type_promote(
                                     variant_index_v<TL, Type_list_gpu>,
                                     variant_index_v<TR, Type_list_gpu>),
                                 Type_list_gpu>;

  template <typename TL, typename TR> struct type_promote_from_gpu_pointer {
    using type = void;
  };
  ...
  // helper typedef
  template <typename TL, typename TR>
  using type_promote_from_gpu_pointer_t =
 ...
 * ... .\n You can use it as following: \code int type = Type.Double; \endcode
 *
